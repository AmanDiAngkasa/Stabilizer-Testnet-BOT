...
def safe_swap(t_in, t_out, lbl, retries=3):
    for attempt in range(retries):
        try:
            nonce = w3.eth.get_transaction_count(account.address, 'pending')
            receipt = run_swap(t_in, t_out, AMOUNT_PER_SWAP, lbl, nonce)

            if receipt.status == 1:
                return True
        except Exception as e:
            print(f"⚠️ Retry {attempt+1} gagal: {e}")
...
def main():
    ...
    for i in range(1, needed_circles + 1):
        ...
        for t_in, t_out, lbl in steps:
            try:
                success = safe_swap(t_in, t_out, lbl)

                if success:
                    print(f"✅ Success: {lbl}")
                else:
                    print(f"❌ Gagal setelah retry: {lbl} → lanjut")
            except Exception as e:
                print(f"❌ Error during {lbl}: {e} → lanjut")
